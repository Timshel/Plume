...
pub fn command() -> Command {
    Command::new("instance").about("Manage instances").subcommand(
        Command::new("new")
            .arg(
                Arg::new("domain")
                    .short('d')
            ...
            .arg(
                Arg::new("name").short('n').long("name").action(clap::ArgAction::Set).help("The name of your instance"),
            )
            .arg(
                Arg::new("default-license")
                    .short('l')
...
fn new(mut args: ArgMatches, conn: &mut Connection) {
    ...
    let name = args.remove_one::<String>("name").unwrap_or_else(|| super::ask_for("Instance name"));
    let license = args.remove_one::<String>("default-license").unwrap_or_else(|| String::from("CC-BY-SA"));
